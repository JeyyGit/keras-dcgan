...
def train(BATCH_SIZE):
    ...
    for epoch in range(100):
        ...
        for index in range(int(X_train.shape[0] / BATCH_SIZE)):
            ...
            g_loss = d_on_g.train_on_batch(noise, np.ones(BATCH_SIZE))[0]

            d.trainable = True
            print("batch %d g_loss : %f" % (index, g_loss))
            if index % 10 == 9:
                g.save_weights("generator.weights.h5", True)
                d.save_weights("discriminator.weights.h5", True)


def generate(BATCH_SIZE, nice=False):
    g = generator_model()
    g.compile(loss="binary_crossentropy", optimizer="SGD")
    g.load_weights("generator.weights.h5")
    if nice:
        d = discriminator_model()
        d.compile(loss="binary_crossentropy", optimizer="SGD")
        d.load_weights("discriminator.weights.h5")
